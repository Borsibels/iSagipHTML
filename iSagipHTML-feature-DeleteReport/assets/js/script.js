...
(function initializeLoginPage() {
  ...
  (function setupAmbulance(){
    ...
    var responderEntries = normalizeResponderEntries(assignment?.responders || raw.responders);
    var responderList = responderEntries.length ? responderEntries.map(function(r){ return r.name; }) : collectResponders({ assignment: assignment, responders: raw.responders, secondaryResponder: raw.secondaryResponder });
    ...
    function renderAssignment(amb) {
      ...
      var responders = responderEntries.map(function(r){ return r.name; });
      var primaryResponder = responderEntries[0];
      var primaryLoc = primaryResponder && primaryResponder.id && responderLocations[primaryResponder.id] ? responderLocations[primaryResponder.id] : null;
      ...
      return (
        '<div class="amb-assignment">'+
          ...
          (primaryLoc && primaryLoc.lat != null && primaryLoc.lng != null
            ? '<p><strong>Primary Responder Location:</strong> '+primaryLoc.lat+', '+primaryLoc.lng+'</p>'
            : '')+
    ...
    async function initializeAmbulanceRealtime() {
      try {
        ...
        // Subscribe to responder locations (assumes responders/{uid} has lat/lng/name)
        var respondersRef = ref(realtimeDb, 'responders');
        onValue(
          respondersRef,
          function(snapshot) {
            var value = snapshot.val() || {};
            responderLocations = value;
            render();
          },
          function(error) {
    ...
    function openAmbulanceModal(amb) {
      ...
      if (ambResponderList) {
        ...
        var responderList = responderEntries.map(function(r){ return r.name; });
        var primaryResponder = responderEntries[0];
        var primaryLoc = primaryResponder && primaryResponder.id && responderLocations[primaryResponder.id] ? responderLocations[primaryResponder.id] : null;
        var extraInfo = primaryLoc && primaryLoc.lat != null && primaryLoc.lng != null
          ? '<li class="muted">Primary responder location: '+primaryLoc.lat+', '+primaryLoc.lng+'</li>'
          : '';
        ambResponderList.innerHTML = responderList.length
          ? responderList.map(function(name){ return '<li>'+name+'</li>'; }).join('') + extraInfo
          : '<li class="muted">No responders recorded</li>';
      }
