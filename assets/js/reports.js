...
(function(){
  ...
  function statusBadge(s){ var map={ Pending:'badge-orange', Relayed:'badge-orange', Responding:'badge-blue', Responded:'badge-green', Resolved:'badge-green' }; return `<span class="badge ${map[s]||'badge-blue'}">${s}</span>`; }
  ...
  function ambulanceOptionsHTML(selected){
    var list=['Ambulance 1','Ambulance 2','Ambulance 3'];
    var html='<option value="">Choose Ambulance</option>';
    for (var i=0;i<list.length;i++){
      var name=list[i];
      var sel=(selected&&selected===name)?' selected':'';
      html+='<option'+sel+'>'+name+'</option>';
    }
    return html;
  }
  ...
  function actionCell(id, status){
    ...
    var selectedAmbulance=(assignments[id]&&assignments[id].ambulanceName)||'';
    var closeBtnHtml = (status!== 'Resolved') ? '<button title="Mark as resolved" class="btn btn-danger close-btn" data-id="'+id+'">RESOLVE</button>' : '';
    ...
    return `
      <div class="actions">
        ...
        <select class="input ambulance-select" data-id="${id}">${ambulanceOptionsHTML(selectedAmbulance)}</select>
        ...
        <button class="btn btn-outline respond-btn" data-id="${id}">${(status==='Responding'?'RESPONDED':'RESPONDING')}</button>
        ${relayBtnHtml}
        ${notifyBtnHtml}
        ${closeBtnHtml}
  ...
  table.addEventListener('click', function(e){
    var btn=e.target.closest('.respond-btn'); if(btn){ var id=btn.getAttribute('data-id'); var r=reports.find(function(x){return x.id===id;}); if(!r) return; r.status=(r.status==='Responding')?'Responded':'Responding'; r.updatedBy='Current User'; r.updatedAt=new Date().toLocaleString(); renderReportsTable(); return; }
    ...
    var close=e.target.closest('.close-btn'); if(close){ var cid=close.getAttribute('data-id'); var idx=reports.findIndex(function(x){return x.id===cid;}); if(idx===-1) return; if(!window.confirm('Mark this report as resolved?')) return; var cr=reports[idx]; cr.status='Resolved'; cr.closedBy='Current User'; cr.updatedBy='Current User'; cr.updatedAt=new Date().toLocaleString(); cr.closedAt=cr.updatedAt; if(!Array.isArray(cr.history)) cr.history=[]; cr.history.push({ ts: cr.updatedAt, user:'Current User', action:'Resolved', details:'Report marked as resolved.' }); archiveReport(cr); reports.splice(idx,1); renderReportsTable(); return; }
    var view=e.target.closest('.view-btn'); if(view){
      ...
      var detailsModal=document.getElementById('modal-details'); var detailsContent=document.getElementById('details-content'); if(detailsContent){ detailsContent.innerHTML=html; } if(detailsModal){ detailsModal.hidden=false; document.body.classList.add('modal-open'); }
      return;
    }
    var closeBtn=e.target.closest('[data-close]'); if(closeBtn){ var m=closeBtn.closest('.modal'); if(m) m.hidden=true; document.body.classList.remove('modal-open'); return; }
  });
  ...
  table.addEventListener('change', function(e){
    ...
    if (rs){
      ...
      if (assigns[rid] && assigns[rid].ambulanceName){ try { window.alert('Assignment successful.'); } catch(e){} }
      renderReportsTable();
      return;
    }
    ...
    var assignsA=loadAssignments(); if(!assignsA[id]) assignsA[id]={}; assignsA[id].ambulanceName=ambName; saveAssignments(assignsA);
    if (assignsA[id] && assignsA[id].responderName){ try { window.alert('Assignment successful.'); } catch(e){} }
  });
  ...
  document.addEventListener('click', function(e){
    if (e.target.classList && e.target.classList.contains('history')){
      ...
      if(historyModal){ historyModal.hidden=false; document.body.classList.add('modal-open'); } return;
    }
    var dc=e.target.closest('[data-close]');
    if (dc){ var modal=dc.closest('.modal'); if(modal) modal.hidden=true; document.body.classList.remove('modal-open'); return; }
  });
  ...
  if (exportReportsBtn){
    exportReportsBtn.addEventListener('click', function(){
      ...
      var body=reports.map(function(r){ var row=[r.description,r.status,r.street,r.landmark,r.photo?'Yes':'No','Ambulance/Severity/Respond/Relay/Notify/View/History/Resolve',r.timestamp,r.responseTime,r.reportedBy,r.closedBy,r.updatedBy,r.updatedAt,r.notes]; return row.map(function(v){ var s=String(v||''); if (s.search(/[",\n]/)>=0) s='"'+s.replace(/"/g,'""')+'"'; return s; }).join(','); }).join('\n');
