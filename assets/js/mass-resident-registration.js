...
(function() {
  ...
  /**
   * Parse CSV content
   * Expected format: Email, Last Name, First Name, Middle Name, Gender, Birth Date, Contact, Address
   */
  function parseCSV(csv) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length < 6) continue; // Skip empty rows (need at least Email, Last Name, First Name, Gender, Birth Date, Contact)

      const email = values[0] || '';
      const lastName = values[1] || '';
      const firstName = values[2] || '';
      const middleName = values[3] || '';
      const gender = values[4] || '';
      const birthDate = values[5] || '';
      const contact = values[6] || '';
      const address = values[7] || '';
      ...
      // Validate entry
      if (!entry.email || !entry.email.includes('@') || !entry.lastName || !entry.firstName || !entry.gender || !entry.birthDate || !entry.contact || !entry.address) {
