...
(function initializeLoginPage() {
  ...
  (function setupStaffRegistration() {
    ...
    registerForm.addEventListener('submit', async function(e) {
      ...
      var gender = document.getElementById('reg-gender').value.trim();
      ...
      // IMMEDIATE FAM BACKGROUND INFO
      var fatherName = document.getElementById('reg-father').value.trim();
      var motherName = document.getElementById('reg-mother').value.trim();

      // ADDRESS INFO
      var homeAddress = document.getElementById('reg-address').value.trim();
      var barangay = document.getElementById('reg-barangay').value.trim();
      var city = document.getElementById('reg-city').value.trim();
      var province = document.getElementById('reg-province').value.trim();
      var zipCode = document.getElementById('reg-zip').value.trim();

      // CONTACT INFO
      var mobileNumber = document.getElementById('reg-mobile').value.trim();
      var email = document.getElementById('reg-email').value.trim().toLowerCase();

      // EMERGENCY CONTACT PERSON
      var emergencyName = document.getElementById('reg-emergency-name').value.trim();
      var emergencyNumber = document.getElementById('reg-emergency-number').value.trim();
      ...
      if (!role || !firstName || !lastName || !birthDate || !fatherName || !motherName || 
          !homeAddress || !barangay || !city || !province || !zipCode ||
          !mobileNumber || !emergencyName || !emergencyNumber ||
          !username || !password) {
      ...
      try {
        ...
        // Use email if provided, otherwise use username@isagip.local format
        var authEmail = email;
        if (!authEmail) {
          authEmail = username + '@isagip.local';
        }
        ...
        var userData = {
          ...
          username: username,
          email: email || null,
          authEmail: authEmail, // Email used for Firebase Auth
          role: role,
          fullName: fullName,
          birthDate: birthDate,
          age: age,
  ...
  (function initializeResidentManagement() {
    ...
    async function openReviewModal(requestId) {
      ...
      document.getElementById('review-username').value = request.username || 'N/A';
    ...
    function openEditModal(residentId) {
      ...
      document.getElementById('edit-username').value = resident.username || '';
